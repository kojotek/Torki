...
public class FuzzyDriver extends Controller {
    ...
    public Action control(SensorModel sensors) {
        ...
        for (int i = 0; i < angles.length; i++) {
            fis.setVariable("track" + i, trackEdge[i]/200.0f);
        }

        double[] curvePredictions = new double[trackEdge.length/2];
        for (int i = 10; i < trackEdge.length; i++) {
            double curvePrediction = (Math.log10(trackEdge[i]/trackEdge[18-i]) - (-2.5f))/(2.5f-(-2.5f));
        ...
        // track axis.
        double angle = (sensors.getAngleToTrackAxis() - 1.0f)/2.0f;
        ...
        toReturn.steering = steering.getValue()  * 2.2f + (-1.1f);
