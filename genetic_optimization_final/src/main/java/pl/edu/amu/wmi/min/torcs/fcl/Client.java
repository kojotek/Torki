...
public class Client {
    ...
    public static void main(String[] args) throws InvalidConfigurationException, ParserConfigurationException, ImproperXMLException, UnsupportedRepresentationException, GeneCreationException, SAXException, IOException {
        ...
        if (accelFis == null) {
            System.err.println("Cannot load FCL driver file: '" + args[1] + "'");
            return;
        }
        
        FIS gearFis = FIS.load(args[2], true);
        if (gearFis == null) {
            System.err.println("Cannot load FCL driver file: '" + args[2] + "'");
            return;
        }

        padDriver driver = new padDriver(steeringFis, accelFis, gearFis, args[0]);
